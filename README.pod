...
I<limebar> [-h | -g I<width>B<x>I<height>B<+>I<x>B<+>I<y> | -c I<command> | -b | -d | -f I<font> | -p | -n I<name> | -u I<pixel> | -B I<color> | -F I<color> | -U I<color>]

=head1 DESCRIPTION

B<limebar> is a lightweight bar based on XCB, cairo and pango. Provides full UTF-8 support, basic formatting, RandR support and EWMH compliance without wasting your precious memory.
...
Modify the text associated to the label.

Example: I<%{Ls:label1}%{A:reboot:} Click here to reboot %{A}%{Ls}> changes the label label1 to a button.
